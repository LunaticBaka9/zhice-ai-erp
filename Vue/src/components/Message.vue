<template>
  <div class="erp-message-center">
    ...
    <div class="message-header">
      ...
      <div class="header-right">
        <el-dropdown trigger="click" @command="handleBatchCommand" :disabled="selectedMessages.length === 0">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="markRead">批量标记已读</el-dropdown-item>
    ...
    <div class="message-layout">
      <!-- 左侧消息列表 -->
      <div class="message-list-panel" :class="{ 'collapsed': !showDetailPanel && isMobile }">
            ...
            >
              ...
              <div class="message-info">
                <div class="message-title">
                  ...
                  <span v-if="message.needAction" class="action-badge">待处理</span>
                </div>
                <div class="message-summary">{{ message.summary }}</div>
                <div class="message-meta">
                  <span class="message-sender">{{ message.sender || '系统管理员' }}</span>
                  <span class="message-time">{{ formatTime(message.time) }}</span>
                  <span v-if="message.deadline" class="message-deadline" :class="{ 'deadline-urgent': isDeadlineUrgent(message.deadline) }">
              ...
              <div class="message-actions" @click.stop>
                <el-tooltip content="标记已读" placement="top" v-if="!message.isRead">
                  <el-button
                      type="primary"
                      :icon="Check"
                      circle
                ...
                <el-tooltip v-if="message.type === 'approval' && !message.isProcessed" content="去审批" placement="top">
                  <el-button
                      type="warning"
                      :icon="Edit"
                      circle
      ...
      <!-- 右侧详情面板 -->
      <div class="detail-panel" :class="{ 'show': showDetailPanel, 'mobile-detail': isMobile }">
        <div class="detail-header">
          ...
          <div class="detail-actions" v-if="selectedMessage">
            ...
            <el-tooltip content="删除" placement="top">
              <el-button :icon="Delete" circle size="small" type="danger" @click="deleteMessage(selectedMessage)" />
            </el-tooltip>
          </div>
        </div>
        <div class="detail-content" v-if="selectedMessage">
          <div class="detail-meta">
            <div class="detail-tags">
              ...
              <el-tag v-if="selectedMessage.needAction" type="warning" size="small">
                需处理
              </el-tag>
          ...
          <div class="detail-info">
            <div class="info-row">
              ...
              <span class="info-value">{{ selectedMessage.sender || '系统管理员' }}</span>
            </div>
            <div class="info-row" v-if="selectedMessage.deadline">
              <span class="info-label">截止时间：</span>
              <span class="info-value" :class="{ 'deadline-urgent-text': isDeadlineUrgent(selectedMessage.deadline) }">
          ...
          <div v-if="selectedMessage.attachments && selectedMessage.attachments.length" class="detail-attachments">
            ...
            <div class="attachments-list">
              <div v-for="(file, idx) in selectedMessage.attachments" :key="idx" class="attachment-item" @click="downloadAttachment(file)">
          ...
          <div class="detail-footer-actions" v-if="selectedMessage.type === 'approval' && !selectedMessage.isProcessed">
...
const generateMockMessages = () => {
  ...
  const templates = {
    system: {
      title: ['系统维护通知', '安全策略更新', '新模块上线通知', '数据库升级预告'],
      summary: ['ERP系统将于本周末进行升级维护，届时将暂停服务2小时', '系统安全策略已更新，请及时修改密码', '采购管理模块已上线，欢迎使用', '数据库将于明日凌晨进行优化升级'],
      sender: '系统运维部'
    },
    approval: {
      title: ['采购申请待审批', '费用报销申请', '合同审批提醒', '请假申请待审批'],
      summary: ['张三提交的采购申请单号PO-2024001，金额¥12,800', '李四提交的费用报销单，金额¥3,200', '销售合同SALE-2024005待您审批', '王五提交的年假申请，共3天'],
      sender: '审批中心'
    },
    todo: {
      title: ['待处理工单提醒', '库存盘点任务', '月度报表待提交', '供应商对账提醒'],
      summary: ['您有3个工单待处理，请及时响应', '仓库A区库存盘点任务已分配给您', '10月份销售报表请于今日下班前提交', '本月供应商对账单已生成，请及时核对'],
      sender: '任务中心'
    },
    finance: {
      title: ['付款审批提醒', '发票到期通知', '预算超支预警', '工资条已生成'],
      summary: ['供应商付款申请单号PAY-2024032待审批', '发票号INV-20241023即将到期，请及时处理', '部门预算使用率已达85%，请注意控制', '本月工资条已生成，请登录查看'],
      sender: '财务部'
    },
    hr: {
      title: ['入职手续提醒', '绩效考核通知', '培训报名确认', '生日祝福'],
      summary: ['新员工王五今日入职，请完成相关手续', '季度绩效考核已开启，请于本周完成自评', '您已成功报名"领导力培训"课程', '祝您生日快乐，公司为您准备了惊喜礼品'],
      sender: '人力资源部'
    }
  }
  ...
  for (let i = 1; i <= 28; i++) {
    ...
    const needAction = type === 'approval' || type === 'todo' ? Math.random() > 0.6 : false
    const time = new Date(now.getTime() - Math.random() * 30 * 24 * 60 * 60 * 1000)

    let deadline = null
    if (needAction && Math.random() > 0.5) {
      deadline = new Date(now.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000)
    }
    ...
    messages.push({
      ...
      deadline: deadline,
      relatedId: type === 'approval' ? `APP-${2024000 + i}` : (type === 'finance' ? `FIN-${2024000 + i}` : null),
...
const pendingApprovalCount = computed(() => messageList.value.filter(msg => msg.type === 'approval' && !msg.isProcessed).length)
...
const clearAllMessages = async () => {
  try {
    await ElMessageBox.confirm('清空后将无法恢复，确定要清空所有消息吗？', '清空确认', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
...
const handleBatchCommand = async (command) => {
  ...
  if (command === 'markRead') {
    ...
    try {
      await ElMessageBox.confirm(`确定要删除选中的 ${selectedMessages.value.length} 条消息吗？`, '批量删除确认', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
      messageList.value = messageList.value.filter(m => !selectedMessages.value.includes(m.id))
      if (selectedMessage.value && selectedMessages.value.includes(selectedMessage.value.id)) {
        selectedMessage.value = filteredMessages.value[0] || null
      }
      selectedMessages.value = []
...
const handleApproval = (message) => {
  ElMessage.info(`跳转到审批页面，审批单号：${message.relatedId || message.id}`)
}
...
const handleSearch = () => {
  currentPage.value = 1
  if (filteredMessages.value.length > 0 && selectedMessage.value && !filteredMessages.value.find(m => m.id === selectedMessage.value.id)) {
...
.message-icon {
  ...
  &.icon-system { background: #e9ecef; color: #495057; }
  &.icon-approval { background: #fff3e0; color: #fd7e14; }
  &.icon-todo { background: #ffe6e6; color: #dc3545; }
  &.icon-finance { background: #e0f7e8; color: #28a745; }
  &.icon-hr { background: #e0e7ff; color: #4f46e5; }
