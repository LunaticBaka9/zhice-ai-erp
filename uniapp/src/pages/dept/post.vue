	...
	<view class="page">
		<view class="search-bar">
			<view class="search-input-wrapper">
				<input class="search-input" v-model="keyword" placeholder="搜索岗位名称" @confirm="handleSearch" />
			</view>
		...
		<view class="toolbar">
			<view class="toolbar-btn primary" @click="handleAdd"><text class="btn-text white">+ 新增岗位</text></view>
		...
		<view class="list">
			...
			<view v-if="list.length === 0 && !loading" class="empty"><text>暂无岗位数据</text></view>
		</view>
		...
		<view v-if="showModal" class="modal-mask" @click="showModal = false">
			<view class="modal-box" @click.stop>
				<text class="modal-title">{{ form.id ? '编辑岗位' : '新增岗位' }}</text>
				<scroll-view scroll-y class="modal-scroll">
					<view class="modal-field">
						<text class="field-label">岗位名称 <text class="required">*</text></text>
						<input class="field-input" v-model="form.name" placeholder="请输入岗位名称" />
					</view>
					<view class="modal-field">
					...
					<view class="modal-field">
						<text class="field-label">岗位简介</text>
						<textarea class="field-textarea" v-model="form.bio" placeholder="请输入岗位简介" :maxlength="200" />
					</view>
				</scroll-view>
...
import { getPostsList, addPost, updatePost, deletePost, updatePostStatus } from '@/api/post'
...
async function submitForm() {
	if (!form.name) { uni.showToast({ title: '请输入岗位名称', icon: 'none' }); return }
	try {
		const fn = form.id ? updatePost : addPost
...
function handleDelete(item) {
	uni.showModal({
		title: '提示', content: `确认删除岗位「${item.name}」？`,
		success: async (res) => {
			if (res.confirm) {
				try {
					const r = await deletePost({ id: item.id })
