<template>
	<view class="page">
		...
		<scroll-view scroll-y class="scroll-area">
			<view class="section">
				<view class="section-title"><text>销售订单 — 按商品汇总（数量 / 金额）</text></view>
				<view v-if="saleOrders.length > 0" class="data-list">
					<view class="data-card" v-for="item in saleOrders" :key="item.goodsId">
...
async function loadData() {
	...
	try {
		...
		saleOrders.value = res.data.saleOrders || []
