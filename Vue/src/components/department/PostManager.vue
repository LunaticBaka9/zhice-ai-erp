    ...
    <div class="user-management">
        ...
        <el-card class="search-card" shadow="never">
            <el-form :inline="true" :model="searchForm" class="search-form">
                <el-form-item label="用户职位名">
                    <el-input
                        v-model="searchForm.name"
                        placeholder="请输入用户职位名"
                        clearable
                        @clear="handleSearch"
        ...
        <el-card class="table-card" shadow="never">
            ...
            >
                ...
                <el-table-column prop="name" label="用户职位" />
        ...
        >
            ...
            >
                ...
                <el-form-item label="用户职位简介" prop="bio">
                    <el-input
                        v-model="dialog.form.bio"
                        type="textarea"
                        placeholder="请输入用户职位简介"
                        :rows="4"
                        maxlength="200"
                        show-word-limit
                        :disabled="dialog.isView"
...
const getPostList = async () => {
    ...
    try {
        ...
        if (res.code === "200") {
            ...
            ElMessage.error(res.msg || "获取用户职位列表失败");
        }
    } catch (error) {
        ElMessage.error("获取用户职位列表失败");
    } finally {
        loading.value = false;
    }
}
        ...
        if (valid) {
            ...
            try {
                ...
                if (dialog.isAdd) {
                    ...
                    if (res.code === "200") {
                        ElMessage.success("新增用户职位成功");
                        dialog.visible = false;
                        getPostList();
                    } else {
                        ElMessage.error(res.msg || "新增用户职位失败");
                    }
                } else {
                    const res = await updatePost(submitData);
                    if (res.code === "200") {
                        ElMessage.success("更新用户职位成功");
                        dialog.visible = false;
                        getPostList();
                    } else {
                        ElMessage.error(res.msg || "更新用户职位失败");
                    }
                }
            } catch (error) {
...
const handleDelete = (row) => {
    ElMessageBox.confirm(`确定要删除用户职位"${row.name}"吗？`, "提示", {
...
const handleStatusChange = async (row) => {
    try {
        const res = await updatePostStatus({ id: row.id, status: row.status });
        if (res.code !== "200") {
