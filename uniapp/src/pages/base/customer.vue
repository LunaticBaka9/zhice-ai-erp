	...
	<view class="page">
		<view class="search-bar">
			<view class="search-input-wrapper">
				...
				<input class="search-input" v-model="keyword" placeholder="搜索客户名称" @confirm="handleSearch" />
			</view>
			<view class="search-btn" @click="handleSearch"><text class="search-btn-text">搜索</text></view>
		</view>
		<view class="toolbar">
			<view class="toolbar-btn primary" @click="handleAdd"><text class="btn-text">+ 新增客户</text></view>
		</view>
		<view class="list">
			...
			<view v-if="list.length === 0 && !loading" class="empty"><text>暂无客户数据</text></view>
		</view>
		...
		<view v-if="showModal" class="modal-mask" @click="showModal = false">
			<view class="modal-box" @click.stop>
				<text class="modal-title">{{ form.id ? '编辑客户' : '新增客户' }}</text>
				<scroll-view scroll-y class="modal-scroll">
					<view class="modal-field">
						<text class="field-label">客户名称 <text class="required">*</text></text>
						<input class="field-input" v-model="form.name" placeholder="请输入客户名称" />
					</view>
					<view class="modal-field">
...
const form = reactive({ id: null, name: '', contactPerson: '', phone: '', email: '', address: '' })

async function loadList() {
	loading.value = true
	try {
		const res = await getCustomerList({ pageNum: pageNum.value, pageSize: pageSize.value, keyword: keyword.value })
		if (res.code === '200') {
...
function handleAdd() {
	Object.assign(form, { id: null, name: '', contactPerson: '', phone: '', email: '', address: '' })
...
function handleDelete(item) {
	uni.showModal({
		title: '提示', content: `确认删除客户【${item.name}】？`,
		success: async (res) => {
			if (res.confirm) {
				try {
...
async function submitForm() {
	if (!form.name) { uni.showToast({ title: '请输入客户名称', icon: 'none' }); return }
	try {
		const res = form.id ? await updateCustomer(form) : await addCustomer(form)
		if (res.code === '200') {
