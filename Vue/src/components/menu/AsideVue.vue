...
import {
  Grid, Setting, User, Lock, Menu, Monitor, UserFilled, Document, Avatar, Message
...
const isTech = computed(() => {
  return data.user?.department === "研发组" || data.user?.department === "测试组" || data.user?.department === "运维组"
})
const isProdect = computed(() => {
  return data.user?.department === "产品组" || data.user?.department==="设计组" || data.user?.department=== "交互组"
})
const isMarket = computed(() => {
  return data.user?.department === "运营组" || data.user?.department==="销售组" || data.user?.department=== "公关组"
})
const isAD = computed(() => {
  return data.user?.department === "人力资源" || data.user?.department==="财务组" || data.user?.department==="办公室"
})
